## Widgets —Day 3 
SolutionsNow build with ASan and run both branches of the dangling-lambda scenario to prove the crash and the fix:Confirmed — "AFTER simulateReading()" prints at `.668`, and the queued slot only fires at `.673`, genuinely after control returned from the emitting call, once the event loop actually started. Now the dangling-lambda crash and its fix:This is a genuinely clean, textbook result:

- **Bug variant**: real ASan heap-use-after-free, pinpointed to the exact line (`mainwindow.cpp:92`, the lambda's `volatileLabel->setText(msg)`), full call stack through `DeviceMonitor::alertRaised` → the dangling lambda → the crash.
- **Fixed variant**: runs completely clean, zero ASan output — because passing `volatileLabel` itself as the context object made Qt auto-disconnect the moment it was deleted, so the lambda never fires at all.
...
signals:
    ...
    void alertRaised(const QString &message);
